...
| `modify` | 注入画布数据，引导模型输出增量操作指令（ModifyPatchOutput） | "把审批节点改红色" |
...
意图识别的输出（intent 类型）决定了后续上下文组装节点是否触发记忆检索：generate 和 modify 需要拉取历史记忆和用户画像，consult 需要历史对话记忆。检索动作发生在上下文组装环节，不在意图节点内执行——意图节点只做路由决策，不直接访问 Store。
