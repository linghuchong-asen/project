...
评估属于测试的核心部分（设定维度与指标），测试还包含工具、场景与压测等。LLM 应用面临黑盒问题，必须可观测、可评估、可调试才能持续迭代。
...
| 组件维度 | 意图识别准确率、规划准确率、记忆召回率、工具调用成功率 | 分阶段统计 |

### 组件评估覆盖范围

组件评估代表功能点评估，应包括：意图识别、工具调用、记忆管理、上下文组装、反思与反馈、模型推理等组件。

### 不同业务场景的指标侧重

| 业务场景 | 核心指标 | 说明 |
|---------|---------|------|
| 推荐系统 | 点击率、转化率、留存率 | 属"完成率"延伸 |
| 安全风控 | 召回率（查全率） | 宁可误杀勿漏放 |
| LLM 问答 | 幻觉率 + 有用性评分 | 比单纯准确率更重要 |
| 代码编写 | 产物正确性、代码可运行性 | — |
...
可观测性为评估提供数据基础，也为定位具体问题提供了可追溯的步骤。

## 7.6 轨迹评估（Trajectory Evaluation）
...
- **问题定位**：某个 case 效果差时，直接看 trace 定位是规划问题、记忆问题还是工具问题
...
- 相关：[部署](./backend/deploy.md) · [工程化踩坑](./engineering.md)
